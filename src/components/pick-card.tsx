...
export function pickTitle(pick: Pick<PickItem, "kind" | "category">): string {
  return `${pick.category === "phone" ? "Phone" : "Laptop"} of the ${pick.kind}`;
}
  ...
  return (
    <article className={cn("flex flex-col border-[3px] border-ink", isYear ? "bg-ink text-paper" : "bg-paper")}>
      <div className={cn("flex items-center justify-between gap-3 border-b-[3px] px-5 py-3", isYear ? "border-paper bg-pink text-ink-deep" : "border-ink")}>
        ...
        <span className="label-mono">{pick.period}</span>
      </div>
      <div className="flex flex-1 flex-col gap-5 p-5 sm:p-6">
        <div className="flex flex-wrap items-start justify-between gap-4">
          ...
          <div className="flex gap-2">
            <ScoreBox score={pick.item.matchScore} label="All-round" tone="match" muted={isYear} />
            <ScoreBox score={pick.item.valueScore} label="Value" tone="value" />
          </div>
        </div>
      ...
      >
        See why it scores {Math.round(pick.item.matchScore)} <ArrowRight className="size-4" aria-hidden />
      </Link>
    </article>
  );
}
